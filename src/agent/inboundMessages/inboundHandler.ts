...
import InboundMessageHandlerInterface, { MessageURI, MessageRouteCallbacks, EncryptedMessage, SignatureDecorator } from './inboundHandlerInterface'
...
class InboundMessageHandler implements InboundMessageHandlerInterface {
    ...
    async newInboundMessage(rawMessage:EncryptedMessage):Promise<void> {
        ...
        let connection:Connection|null = await this._findConnection(processedMessage.recipient_verkey)

        if(Connection.guard(connection)){
            console.info(`New message for Connection ${connection.connectionID} of type ${messageType}`, connection)
        }
        else{
            if(messageType !== ("https://didcomm.org/connections/1.0/request" || "did:sov:BzCbsNYhMrjHiqZDTUASHg;spec/connections/1.0/request")){
                console.warn(`Unable to find connection for new message with processedMessage:`, processedMessage)
            }
        }
    ...
    async _verifySignatures(message:any):Promise<any> {
        ...
        for (const key of Object.keys(message)) {
            ...
            if (signatureDecorator && signatureRegexGroups) {
                ...
                const signature:number[] = Array.from(base64url.toBuffer(signatureField.signature))
                const signedData:number[] = Array.from(base64url.toBuffer(signatureField.sig_data))

                const valid = await WalletService.verifyData(signerVerkey, signedData, signature)
    ...
    async _findConnection(recipientVerkey:string):Promise<Connection|null> {
        ...
        const connections:any = await connectionsService.searchConnectionRecords([recipientVerkey])
    ...
    async _broadcastToHandlers(messageType:string, unpackedMessage:{}, connection:Connection|null):Promise<void> {
