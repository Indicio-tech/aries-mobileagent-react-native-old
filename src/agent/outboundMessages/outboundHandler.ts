...
import OutBoundMessageHandlerInterface, { EncryptedMessage } from './outboundHandlerInterface'
...
class OutBoundMessageHandler implements OutBoundMessageHandlerInterface {
    ...
    async sendMessage(
        ...
        await this._sendMessageToTransport(packedMessage, endpoint, inboundMessagesCallback)
    }

    async _sendMessageToTransport(packedMessage:Buffer, endpoint:string, inboundMessagesCallback:Function):Promise<void> {
        let endpointURL = url.parse(endpoint)

        switch(endpointURL.protocol){
            case 'https:':
            case 'http:':
                await HTTP.sendOutboundMessage(packedMessage, endpoint, inboundMessagesCallback)
                break
            case 'wss:':
            case 'ws:':
                await WS.sendOutboundMessage(packedMessage, endpoint, inboundMessagesCallback)
                break
            default:
                throw new Error(`Unidentified Transport Protocol ${endpointURL.protocol}`)
        }
    }
    ...
    async sendMessageByConnectionID(
        ...
        const connection:Connection = await connectionsService.getConnectionRecord(connectionID)
