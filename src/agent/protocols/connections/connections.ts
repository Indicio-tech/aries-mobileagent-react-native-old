...
import { Connection, InvitationRecord, Service } from '../../services/connections/connectionsServiceInterface'
...
import { Invitation, PublicDIDInvitation, ConnectionRequest, ConnectionResponse } from './messages'

class ConnectionsHandler extends BaseProtocolHandler implements ConnectionsHandlerInterface {
    messageCallbacks = {
        "did:sov:BzCbsNYhMrjHiqZDTUASHg;spec/connections/1.0/response": this.receiveResponse.bind(this)
    }
    constructor(){
    ...
    async sendRequest(
        ...
        await OutboundMessageHandler.sendMessage(endpoint, request, recipientKeys, senderVerkey, routingKeys)
    }

    async receiveResponse(unpackedMessage:any, connection:Connection):Promise<void> {
        console.info("Received new connection response", unpackedMessage, connection)
        
        ConnectionResponse.check(unpackedMessage.message)

        //Retrieve the connection invitation
        const invitation:InvitationRecord = await ConnectionsService.getInvitationRecord(connection.invitationID)
        ...
        for (var i = 0; i < invitation.service.recipientKeys.length; i++) {
            if (invitation.service.recipientKeys[i] === unpackedMessage.message['connection~sig'].signer) {
                matching = true 
                break;
            }
        }
        ...
        await ConnectionsService.receiveResponse(unpackedMessage.message.connection.DIDDoc, connection)
    }
}
