...
import ConnectionsServiceInterface, {
  ConnectionsAdminAPIInterface,
  InvitationRecord,
  Service,
  InvitationUses,
  InvitationID,
  ...
  ConnectionRoles,
  ConnectionStates,
} from './connectionsServiceInterface'
...
class ConnectionsService implements ConnectionsServiceInterface {
  ...
  async acceptInvitation(
    ...
    const invitation: InvitationRecord = await this.getInvitationRecord(
      invitationID,
    )
    ...
    if (invitation.role === 'peer') {
      if (!mediatorID) {
        throw new Error(
          `Connection Role "peer" requires a mediatorID so connections can be mediated`,
        )
      }

      const mediatorRecord: MediatorRecord = await MediationService.getMediatorRecord(
        mediatorID,
      )
    ...
    const didDOC = await DIDService.createDIDDoc(
      didKeyPair,
      endpoint,
      routingKeys,
    )
  ...
  async connectionMediated(connection: Connection): Promise<void> {
    ...
    if (connection.state === 'invite-received') {
      ...
      throw new Error(
        `Unable to proceed after connection mediated with state ${connection.state}`,
      )
    }
  }
  ...
  async receiveResponse(didDoc: DIDDoc, connection: Connection): Promise<void> {
    console.info(
      `Received Connection Response for connection ${connection.connectionID}`,
    )
    ...
    const updatedConnection: Connection = await this.updateConnectionRecord(
  ...
  async updateConnectionRecord(
    ...
    let connection: Connection = Object.assign(
      currentConnection,
      connectionUpdates,
    )
    connection.updatedAt = Date.now().toString()

    await StorageService.updateRecord('connection', connectionID, connection)
  ...
  async _broadcastConnectionRecord(connection: Connection): Promise<void> {
    console.info(
      `Connection Record Update for Connection ${connection.connectionID}`,
      connection,
    )
  }
}
