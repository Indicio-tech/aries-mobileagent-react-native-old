...
import DIDServiceInterface, {DIDKeyPair, DIDDoc, ServiceEndpoint, Verkey, Service} from './didServiceInterface'
...
class DIDService implements DIDServiceInterface {
    ...
    async retrieveDIDDocService(didDoc:DIDDoc):Promise<Service> {
        ...
        for(var i = 0; i < didDoc.service.length; i++){
            ...
            let potentialServiceTransport = url.parse(potentialService.serviceEndpoint)
            
            if(potentialService.priority < selectedService.priority){
                selectedService = potentialService
            }
            else if(potentialService.priority === selectedService.priority 
                && (potentialServiceTransport.protocol === 'ws:' || potentialServiceTransport.protocol === 'wss:')){
                    selectedService = potentialService
            }
        }
