...
import MediationServiceInterface, {
  MediationAdminAPIInterface,
  MediatorInvite,
  MediatorID,
  MediatorRecord,
  KeylistUpdate,
  KeylistUpdateResults,
} from './mediationServiceInterface'
...
class MediationService implements MediationServiceInterface {
  ...
  constructor() {
    ...
    WS.webSocketCloseEvents.addListener(
      'handler-closed',
      this._webSocketHandlerClosed.bind(this),
    )
  }
  ...
  async _webSocketHandlerClosed(endpoint: string): Promise<void> {
    ...
    if (mediatorRecords[0]) {
      ...
      console.warn(
        `Mediator ${mediator.mediatorID} WebSocket Handler closed, sending trust ping to re-open connection`,
      )
  ...
  async addMediator(
    ...
    console.info(
      `Mediation Connection created with connection ID: ${connectionID}`,
    )
  }
  ...
  async requestMediation(connection: Connection): Promise<void> {
    ...
    await CoordinateMediationHandler.sendMediationRequest(
    ...
    await StorageService.addTagsToRecord('mediator', connection.mediatorID, {
      endpoint: connection.externalService.serviceEndpoint,
    })
  }

  async mediationGranted(
  ...
  async sendKeylistUpdate(
    ...
    console.info(
      `Sending keylist update to mediator ${mediatorID}`,
      keylistUpdates,
    )

    const mediatorRecord = await this.getMediatorRecord(mediatorID)

    await CoordinateMediationHandler.sendKeylistUpdate(
      mediatorRecord.connectionID,
      keylistUpdates,
    )
  }
  ...
  async receiveKeylistUpdateResponse(
    mediatorID: MediatorID,
    keylistUpdatesResults: KeylistUpdateResults[] = [],
  ): Promise<void> {
    console.info(`Received Keylist Update Response from mediator ${mediatorID}`)

    for (var i = 0; i < keylistUpdatesResults.length; i++) {
      if (
        keylistUpdatesResults[i].result === 'client_error' ||
        keylistUpdatesResults[i].result === 'server_error'
      ) {
        ...
        const connectionsRecords = await ConnectionsService.searchConnectionRecords(
          [keylistUpdatesResults[i].recipient_key],
        )
        ...
        console.warn(
          `Unrecognized Keylist update response result ${keylistUpdatesResults[i].result}`,
        )
      }
    }
  }

  async _addMediatorRecord(
    mediatorRecord: MediatorRecord,
    tags: {} = {},
  ): Promise<void> {
  ...
  async updateMediationRecord(
    ...
    let mediatorRecord: MediatorRecord = Object.assign(
      currentMediatorRecord,
      mediatorUpdates,
    )

    await StorageService.updateRecord('mediator', mediatorID, mediatorRecord)
  ...
  async _broadcastMediationRecord(
    mediatorRecord: MediatorRecord,
  ): Promise<void> {
    console.info(
      `Mediation Record Update for Mediator ${mediatorRecord.mediatorID}`,
      mediatorRecord,
    )
  }
}
