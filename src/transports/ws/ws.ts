...
class WS implements WSInterface {
    ...
    async _addWebSocketHandler(endpoint:string, messageBuffer:Buffer, inboundMessagesCallback:Function):Promise<void> {
        ...
        this.webSocketHandlers[endpoint].onclose = async (closeEvent) => {
            console.info(`${endpoint} - WebSocket Handler closed with closeEvent:`, closeEvent)
    ...
    async sendOutboundMessage(messageBuffer:Buffer, endpoint:string, inboundMessagesCallback:Function):Promise<void> {
        ...
        if(this.webSocketHandlers[endpoint]){
            ...
            await this._addWebSocketHandler(endpoint, messageBuffer, inboundMessagesCallback)
        }
    }
}
