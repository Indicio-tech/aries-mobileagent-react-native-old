import base64url from 'base64url'

//Encode and decode Base64
export const decodeBase64 = function(encodedMessage:string):string {
    return Buffer.from(encodedMessage, 'base64').toString('ascii')
}
  
export const encodeBase64 = function(message:string):string {
    return Buffer.from(message).toString('base64')
}

//Encode and decode Base64URL
// export const decodeBase64URL = function(encodedMessage:string):string {
//     return base64url.toBuffer(encodedMessage)
// }

export const encodeBase64URL = function(message:string):string {
    return base64url.encode(message)
}