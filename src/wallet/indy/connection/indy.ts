...
export default class IndyConnection implements IndyInterface {
  ...
  async createWallet(
    ...
    try {
      ...
      const walletConfig = this._createWalletConfig(
        walletName,
        defaultStoragePath,
      )
  ...
  async openWallet(
    ...
    try {
      ...
      //Create the Wallet
      await Indy.checkWallet(walletConfig, walletCredentials)

      console.info('Wallet has been opened')
  ...
  async createPoolLedgerConfig(
    ...
    try {
      ...
      await Indy.createPoolLedgerConfig(configName, poolConfig)
  ...
  async openPool(
    configName: PoolConfigName,
    runtimeConfig: PoolRuntimeConfig = null,
  ): Promise<void> {
    try {
      ...
      //Open up the pool
      await Indy.checkPool(configName, runtimeConfig)
  ...
  async verify(
    ...
    try {
      ...
      //Pack Message
      let verified: boolean = await Indy.cryptoVerify(
        signerVerkey,
        signedData,
        signature,
      )
  ...
  async searchRecords(
    ...
    try {
      console.info(
        `Searching for records by query ${query} and type ${recordType}`,
      )
  ...
  async addTags(
    ...
    try {
      console.info(`Adding Tags to Record`)
