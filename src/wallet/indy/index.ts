...
class IndyService implements WalletServiceInterface {
  ...
  async createWallet(
    ...
    await this.#indy.createMasterSecretID(
      walletName,
      walletPassword,
      masterSecretID,
    )
    ...
    await this.#indy.createPoolLedgerConfig(
      ledgerConfigName,
      ledgerGenesisString,
    )
  ...
  async unpackMessage(message: string): Promise<string> {
